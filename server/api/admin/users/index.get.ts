...
export default defineEventHandler(async (event) => {
  ...
  if (query.role) params.append('role', String(query.role))
  if (query.status) params.append('status', String(query.status))

  const urlString = `http://localhost/api/admin/users?${params.toString()}`
  ...
  return new Promise((resolve, reject) => {
    const req = http.request({
      ...
      reject(createError({
        statusCode: 502,
        statusMessage: 'Failed to fetch users: ' + e.message
      }))
    })
    req.end()
  })
