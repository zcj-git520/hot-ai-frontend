...
const loadList = async () => {
  loading.value = true
  try {
    // Try to use the adminApi first
    const response = await adminApi.user?.getList?.({ page: currentPage.value, pageSize })
...
const handleUpdateRole = async () => {
  ...
  try {
    ...
    console.log('[UsersContent] Update role mock:', userId, role)
    // Mock success
    toastSuccess(`用户角色已更新为 ${role === 'admin' ? '管理员' : '普通用户'}`)
...
const handleDisable = async (id: number) => {
  try {
    ...
    console.log('[UsersContent] Disable user mock:', id)
    toastSuccess('用户已禁用')
  }
...
const handleEnable = async (id: number) => {
  try {
    ...
    console.log('[UsersContent] Enable user mock:', id)
    toastSuccess('用户已启用')
  }
...
const openLogsModal = async (user: UserListItem) => {
  ...
  try {
    ...
    console.log('[UsersContent] Get logs mock:', user.id)
    // Mock logs filtered by user
    logs.value = mockLogs.filter(log => log.user_id === user.id)
  } finally {
