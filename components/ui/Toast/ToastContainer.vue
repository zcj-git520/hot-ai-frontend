    ...
    <div id="toast-container" class="fixed top-20 right-4 z-50">
      <TransitionGroup name="toast">
        <div 
          ...
          :key="toast.id"
          :class="[
            'rounded-lg shadow-lg p-4 mb-3 max-w-sm w-full flex items-start gap-3',
            'bg-opacity-20 border backdrop-blur-sm',
            toast.type === 'success' ? 'bg-green-900 border-green-800' : '',
            toast.type === 'error' ? 'bg-red-900 border-red-800' : '',
            toast.type === 'info' ? 'bg-blue-900 border-blue-800' : '',
            toast.type === 'warning' ? 'bg-yellow-900 border-yellow-800' : ''
          ]"
        >
          <!-- 成功图标 -->
...
onUnmounted(() => {
  window.removeEventListener('toast:add', (event: any) => {
    removeToast(event.detail.id)
  })
  window.removeEventListener('toast:remove', (event: any) => {
    removeToast(event.detail.id)
  })
})
</script>
